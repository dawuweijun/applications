...
#ifdef USE_DOUBLE
	__global__ void copy(double *temp, double *old, unsigned int size)
#else
	__global__ void copy(float  *temp, float  *old, unsigned int size)
#endif
	{
...
#ifdef USE_DOUBLE
	__global__ void blend(double *temp, double *old, unsigned int size)
#else
	__global__ void blend(float  *temp, float  *old, unsigned int size)
#endif
	{
		unsigned int offset = blockIdx.x * blockDim.x + threadIdx.x;
...
int main(int argc, char **argv){
	...
	if(checkFlag(argv, argc, "debug")){
		#ifdef USE_DOUBLE
			printf("Heat Equation - Vector - CUDA - Double Precision\n");
		#else
			printf("Heat Equation - Vector - CUDA - Single Precision\n");
		#endif
	...
	// Blocks and Grids.
	unsigned int BLOCK_COPY  = 512;
	unsigned int BLOCK_BLEND = 512;
	
	unsigned int GRID_COPY  = (size * size + BLOCK_COPY - 1 ) / BLOCK_COPY ;
	unsigned int GRID_BLEND = (size * size + BLOCK_BLEND - 1) / BLOCK_BLEND;
	...
	// Memory alloc
	#ifdef USE_DOUBLE
		GPU_HANDLE_ERROR(cudaMalloc((void **) &d_old,  size * size * sizeof(double)));
		GPU_HANDLE_ERROR(cudaMalloc((void **) &d_temp, size * size * sizeof(double)));
	#else
		GPU_HANDLE_ERROR(cudaMalloc((void **) &d_old,  size * size * sizeof(float)));
		GPU_HANDLE_ERROR(cudaMalloc((void **) &d_temp, size * size * sizeof(float)));
	#endif

	// Memory Copy
	#ifdef USE_DOUBLE
		GPU_HANDLE_ERROR(cudaMemcpy(d_old,  old,  size * size * sizeof(double), cudaMemcpyHostToDevice));
		GPU_HANDLE_ERROR(cudaMemcpy(d_temp, temp, size * size * sizeof(double), cudaMemcpyHostToDevice));
	#else
		GPU_HANDLE_ERROR(cudaMemcpy(d_old,  old,  size * size * sizeof(float),  cudaMemcpyHostToDevice));
		GPU_HANDLE_ERROR(cudaMemcpy(d_temp, temp, size * size * sizeof(float),  cudaMemcpyHostToDevice));
	#endif
	...
	for(time = 1; time <= iterations; time++){
		GPU_HANDLE_ERROR(cudaEventRecord(start[1], 0));
	    	copy<<<GRID_COPY, BLOCK_COPY>>>(d_temp, d_old, size);
	    ...
	    GPU_HANDLE_ERROR(cudaEventRecord(stop[1], 0));	GPU_HANDLE_ERROR(cudaEventSynchronize(stop[1]));
  		GPU_HANDLE_ERROR(cudaEventElapsedTime(&timer[TEMP], start[1], stop[1]));
	    timer[COPY] += timer[TEMP];

	    GPU_HANDLE_ERROR(cudaEventRecord(start[1], 0));
	    	blend<<<GRID_BLEND, BLOCK_BLEND>>>(d_temp, d_old, size);
	    GPU_HANDLE_ERROR(cudaDeviceSynchronize()); GPU_HANDLE_ERROR(cudaGetLastError());
	    GPU_HANDLE_ERROR(cudaEventRecord(stop[1], 0));	GPU_HANDLE_ERROR(cudaEventSynchronize(stop[1]));
  		GPU_HANDLE_ERROR(cudaEventElapsedTime(&timer[TEMP], start[1], stop[1]));
	...
	GPU_HANDLE_ERROR(cudaEventRecord(stop[0], 0));	GPU_HANDLE_ERROR(cudaEventSynchronize(stop[0]));
  	GPU_HANDLE_ERROR(cudaDeviceSynchronize()); GPU_HANDLE_ERROR(cudaGetLastError());
    GPU_HANDLE_ERROR(cudaEventElapsedTime(&timer[TOTAL], start[0], stop[0]));
	...
	// Copy device to host
	#ifdef USE_DOUBLE
		GPU_HANDLE_ERROR(cudaMemcpy(temp, d_temp, size * size * sizeof(double), cudaMemcpyDeviceToHost));
	#else
		GPU_HANDLE_ERROR(cudaMemcpy(temp, d_temp, size * size * sizeof(float),  cudaMemcpyDeviceToHost));
	#endif
	...
	GPU_HANDLE_ERROR(cudaFree(d_old));
