...
enum Heat{COPY = 0, BLEND, TEMP, TOTAL};
...
#endif
	{
		...
		cilk_for(offset = 0; offset < size * size; offset++){
...
#endif
	{
		...
		cilk_for(offset = 0; offset < size * size; offset++){
			unsigned int x = offset / size;
			unsigned int y = offset % size;

			unsigned int left  = offset - 1;
		    ...
		    unsigned int top    = offset - size;
		    unsigned int bottom = offset + size;
...
int main(int argc, char **argv){
	...
	if(checkFlag(argv, argc, "debug")){
		#ifdef USE_DOUBLE
			printf("Heat Equation - Vector - Cilk - Threads: %d - Double Precision\n", __cilkrts_get_nworkers());
		#else
			printf("Heat Equation - Vector - Cilk - Threads: %d - Single Precision\n", __cilkrts_get_nworkers());
		#endif
