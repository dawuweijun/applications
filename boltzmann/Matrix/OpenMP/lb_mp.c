...
void redistribute(s_lattice *l){
	...
	#pragma omp parallel for default(shared) private(x, y, z)
	for(x = 0; x < l->lx; x++)
		for(y = 0; y < l->ly; y++)
			for(z = 0; z < l->lz; z++)
...
void propagate(s_lattice *l){
	...
	#pragma omp parallel for default(shared) private(x, y, z, x_e, y_u, z_n, x_w, y_d, z_s)
	for(x = 0; x < l->lx; x++)
		for(y = 0; y < l->ly; y++)
			for(z = 0; z < l->lz; z++){
...
void relaxation(s_lattice *l){
	...
	#pragma omp parallel for default(shared) private(x, y, z, d_loc, u_x, u_y, u_z, u_squ, u_n, n_equ)
	for(x = 0; x < l->lx; x++){
		for(y = 0; y < l->ly; y++){
			for(z = 0; z < l->lz; z++){
				if(l->obst[x][y][z] == false){
...
int main(int argc, char **argv){
	...
	if(checkFlag(argv, argc, "debug")){
			...
			#pragma omp parallel
			#pragma omp single
			printf("Lattice Boltzmann Method - D3Q19 - Matrix - OpenMP - Threads: %d - Double Precision\n", omp_get_num_threads());
		#else
			#pragma omp parallel
			#pragma omp single
			printf("Lattice Boltzmann Method - D3Q19 - Matrix - OpenMP - Threads: %d -Single Precision\n", omp_get_num_threads());
		#endif
