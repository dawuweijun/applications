...
void alloc_lattice(s_lattice *l, unsigned short int debug){
	...
	l->obst = (unsigned short int ***) calloc(l->lx, sizeof(unsigned short int **));
	#ifdef USE_DOUBLE
		l->node = (double ****) calloc(l->lx, sizeof(double***));
		l->temp = (double ****) calloc(l->lx, sizeof(double***));
	#else
		l->node = (float ****) calloc(l->lx, sizeof(float***));
		l->temp = (float ****) calloc(l->lx, sizeof(float***));		
	#endif

	for(x = 0; x < l->lx; x++){
		l->obst[x] = (unsigned short int **) calloc(l->ly, sizeof(unsigned short int *));
		#ifdef USE_DOUBLE
			l->node[x] = (double ***) calloc(l->ly, sizeof(double**));
			l->temp[x] = (double ***) calloc(l->ly, sizeof(double**));
		#else
			l->node[x] = (float ***) calloc(l->ly, sizeof(float**));
			l->temp[x] = (float ***) calloc(l->ly, sizeof(float**));
		#endif
		for(y = 0; y < l->ly; y++){
			l->obst[x][y] = (unsigned short int *) calloc(l->lz, sizeof(unsigned short int));
			#ifdef USE_DOUBLE
				l->node[x][y] = (double **) calloc(l->lz, sizeof(double*));
				l->temp[x][y] = (double **) calloc(l->lz, sizeof(double*));
			#else
				l->node[x][y] = (float **) calloc(l->lz, sizeof(float*));
				l->temp[x][y] = (float **) calloc(l->lz, sizeof(float*));				
			#endif
			for(z = 0; z < l->lz; z++){
				#ifdef USE_DOUBLE
					l->node[x][y][z] = (double *) calloc(NDIM, sizeof(double));
					l->temp[x][y][z] = (double *) calloc(NDIM, sizeof(double));
				#else
					l->node[x][y][z] = (float *) calloc(NDIM, sizeof(float));
					l->temp[x][y][z] = (float *) calloc(NDIM, sizeof(float));					
				#endif
			}
		}
	}
		...
		for(y = 0; y < l->ly; y++)
			for(z = 0; z < l->lz; z++)
				if (l->obst[x][y][z] == false){
					...
					u_x += (l->node[x][y][z][1] + l->node[x][y][z][2] + l->node[x][y][z][8] + l->node[x][y][z][9] + l->node[x][y][z][14]  - (l->node[x][y][z][4] + l->node[x][y][z][5] + l->node[x][y][z][6] + l->node[x][y][z][11] + l->node[x][y][z][13])) / d_loc;
					n_free++;
				}
			...
			for(y = 0; y < l->ly; y++){
				for(x = 0; x < l->lx; x++){
					if (l->obst[x][y][z] == true){
						...
						d_loc = 0.0;
						for(i = 0; i < NDIM; i++)
							d_loc += l->node[x][y][z][i]; /* integral local density */
